...
- (void)viewDidLoad {
    ...
    /* 选项卡的ViewControllers*/
    [self setViewControllers:@[indexPageVC,tutoriumVC,classTimeVC,noticeVC,personalVC]];
    ...
    for (int i=0; i<5; i++) {
        ...
        RDVTabBarItem *item=[[RDVTabBarItem alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.tabBar.frame)/5*i, 0, CGRectGetWidth(self.tabBar.frame)/5, CGRectGetHeight(self.tabBar.frame))];
...
- (void)badgeShows{
    ...
    if ([self.rdv_tabBarController.tabBar.items[3] badge]==nil) {
        
        [self.rdv_tabBarController.tabBar.items[3] showBadges];
        
    }else{
        
        [[self.rdv_tabBarController.tabBar.items[3] badge]setHidden:NO];
        [self.rdv_tabBarController.tabBar.items[3] setIsShowNow:YES];
...
- (void)badgeHides{
    ...
    [self.rdv_tabBarController.tabBar.items[3] hideBadges];
    [self.rdv_tabBarController.tabBar.items[3] setIsShowNow:NO];
    [[self.rdv_tabBarController.tabBar.items[3] badge]setHidden:YES];

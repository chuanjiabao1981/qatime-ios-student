...
- (void)drawView {
    if (!_paymentAlert) {
        ...
        [_paymentAlert addSubview:_headImage];
        [_headImage sd_setImageWithURL:[NSURL URLWithString:[[NSUserDefaults standardUserDefaults] valueForKey:@"avatar_url"]]];
        _headImage.layer.masksToBounds = YES;
        _headImage.layer.cornerRadius =TITLE_HEIGHT/2-5;
        
        _closeBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_closeBtn setFrame:CGRectMake(_paymentAlert.width_sd-TITLE_HEIGHT, 0, TITLE_HEIGHT, TITLE_HEIGHT)];
...
/* 类方法*/
+(DCPaymentView *)showPayAlertWithTitle:(NSString * _Nullable)title andDetail:(NSString * _Nullable)detail andAmount:(CGFloat)amount completeHandle:(completedHandle)handel{
    
    DCPaymentView *alert = [[DCPaymentView alloc]init];
    alert.titleStr = title;
    alert.detail = detail;
    alert.amount = amount;
    [alert show];
    alert.completeHandle = ^(NSString *inputpwd){
